...
{
    return "/portfolio/$lang->name/$target_page_name.html";
}

function put_regular_page(Lang $lang, string $page_name, callable $put_main, string $fallback_stylesheet = 'base.css')
{
put_doctype_html($lang);
put_head($lang, $page_name, $fallback_stylesheet);
?>

<body>
    <?php put_header($lang, $page_name) ?>
    <?php $put_main() ?>
    <?php put_footer($lang) ?>
</body>
...
function put_doctype_html(Lang $lang)
{
?>
<!DOCTYPE html>
<html lang="<?= $lang->key ?>">
...
function put_head(Lang $lang, string $page_name, string $fallback_stylesheet = 'base.css')
{
?>
...
<head>
    ...
    <?php
    $stylesheet_url = 'portfolio/css/' . $page_name . '.css';
    if (file_exists(root_path($stylesheet_url))) {
?>
    <link rel="stylesheet" type="text/css" href="/<?= $stylesheet_url ?>">
    <?php } else { ?>
    <link rel="stylesheet" type="text/css" href="/portfolio/css/<?= $fallback_stylesheet ?>">
    <?php } ?>
    <meta charset="UTF-8">
    ...
    <!-- JQuery -->
    <script async src="/portfolio/js/jquery-3.7.1.slim.min.js" integrity="sha256-kmHvs0B+OpCW5GVHUNjv9rOmY0IvSIRcf7zGUDTDQM8=" crossorigin="anonymous"></script>
    <!-- Highlight.JS -->
    ...
    <?php
    $script_url = "portfolio/js/$page_name.js";
    if (file_exists(root_path($script_url))) {
?>
...
function put_header(Lang $lang, string $page_name)
{
?>
...
<header class="lvl">
    <nav>
        <a href="<?= nav_href($lang, 'index') ?>">Raphaël Bardini</a>
        <ul>
            <li><a href="<?= nav_href($lang, 'projects') ?>"><?= $lang->get('namePageProjects') ?></a></li>
            <li><a href="<?= nav_href($lang, 'history') ?>"><?= $lang->get('namePageHistory') ?></a></li>
            <li><a href="<?= nav_href($lang, 'passions') ?>"><?= $lang->get('namePagePassions') ?></a></li>
            <li><a href="<?= nav_href($lang, 'perspectives') ?>"><?= $lang->get('namePagePerspectives') ?></a>
            <li><a href="<?= nav_href($lang, 'but-informatique') ?>"><?= $lang->get('namePageButInformatique') ?></a></li>
        </ul>
    </nav>
    <ul class="list-flags">
        <?php
    ...
    ?><li><a class="<?= $class ?>" title="<?= $title ?>" href="/portfolio/<?= $otherLang->name ?>/<?= $page_name ?>.html"></a></li><?php
    }
?>
    </ul>
