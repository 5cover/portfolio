...
type CopyNode = { title: string; abbr: Copy } | { datetime: Date; time: Copy };
...
class Parser<Jsx extends JsxFn> {
    ...
    parse(c: Copy): ReturnType<Jsx> | null {
        if (c === null) return null;
        if (typeof c === 'string') return this.jsx(this.fragment, { children: c }) as ReturnType<Jsx>;
        if (Array.isArray(c)) {
            return this.jsx(this.fragment, { children: c.map(r => this.parse(r)) }) as ReturnType<Jsx>;
        }
    ...
    private node<K extends string>(c: Record<K, Copy>, k: K, attrs: Record<string, string>) {
        return this.jsx(k, { ...attrs, children: this.parse(c[k]) }) as ReturnType<Jsx>;
    }
}
