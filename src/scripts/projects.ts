    ...
    const links = Array.isArray(entry.links)
      ? entry.links
          .map((link) => {
            ...
            if (typeof link.label !== 'string' || typeof link.href !== 'string' || typeof link.anchor !== 'string') {
              return null;
            }
            return {
              label: link.label,
...
async function getGraphicMarkup(
  url: string,
  isThemedSvg: boolean,
  title: string,
  className?: string
...
async function renderProjectCard(
  ...
  const logoHtml = project.logo
    ? await getGraphicMarkup(
        project.logo.url,
        project.logo.isThemedSvg,
        stripTags(project.title),
        'logo'
      )
    : '';
